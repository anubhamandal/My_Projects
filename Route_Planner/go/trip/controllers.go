...
func (u *UberService) findBestRoute(request TripsRequest) (Plan, error) {
	...
	if request.Ending_location_id != request.Starting_from_location_id {
		ids = append(ids, request.Ending_location_id)
	}
	...
	if request.Ending_location_id != request.Starting_from_location_id {
		for i := 0; i < len(destinations); i++ {
			if destinations[i].Id == request.Ending_location_id {
				end_location = destinations[i]
				destinations = append(destinations[:i], destinations[i+1:]...)
				break
			}
		}
	} else {
		end_location.Id = start_location.Id
		end_location.Name = start_location.Name
		end_location.Address = start_location.Address
		end_location.City = start_location.City
		end_location.State = start_location.State
		end_location.Zip = start_location.Zip
		end_location.Coordinate = start_location.Coordinate
	}
	//	fmt.Println(start_location)
	//	fmt.Println(destinations)
	//	fmt.Println(end_location)
	// Find shortest path
	best_path := find_shortest_path(start_location, end_location, destinations)
	...
	// Calculations for last leg of trip
	...
	plan.Ending_location_id = request.Ending_location_id
